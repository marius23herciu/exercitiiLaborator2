...
namespace exercitiiLaborator2
{
    class Program
    {
        ...
        static void Ex2()
        {
            ...
            //Varianta a 2a cu while

            Console.WriteLine("Introduceti un sir de numere despartite de spatiu.");
            string numere = (Console.ReadLine());
            string cifrePlusSpatiu = "01234 56789";
            for (int i = 0; i < numere.Length; i++)
            {
            ...
            for (int i = 0; i < numere.Length; i++)
            {
                ...
                {
                    sirTemporar = sirTemporar + numere[j].ToString();
                    if (j == i - 1)
                    {
                        int numarSir = int.Parse(sirTemporar);
                        if (numarSir % 2 == 0)
                        {
                            suma += numarSir;
                            numerotareNumerePare++;
                        }
                    }
                }
            }
            if (numerotareNumerePare < 2)
            {
                Console.WriteLine("Nu au fost introduse cel putin 2 numere pare.");
                return;
            }
            medieAritmetica = suma / numerotareNumerePare;
            Console.WriteLine("Media aritmetica a numerelor pare introduse este " + medieAritmetica);
        }
        static void Ex3()
        {
